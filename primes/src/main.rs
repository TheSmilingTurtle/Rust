fn main() {
    let number: usize = {
        std::io::Write::write_all(&mut std::io::stdout(), b"Enter a number: ").expect("Could not prompt user"); //prompt user
        ...
        let _ = std::io::stdin().read_line(&mut buffer).expect("Failed to read line"); //read into string

        buffer.trim().parse::<usize>().expect("Could not parse input") //parse from string and store in number
    };
    ...
    let clean_vec: Vec<usize> = {
        ...
        for pos in 0..lim{
            ...
            if jump != 0{ // jump = 0 means we have already visited this position
                dirty_vec.iter_mut()
    ...
    if number <= 100{
        std::io::Write::write_all(&mut std::io::stdout(), ("Here are your primes: \n".to_owned() + &format!("{:?}", clean_vec) + "\n").as_bytes()).expect("Could not post to stdout");
    }
    std::io::Write::write_all(&mut std::io::stdout(), ("There are ".to_owned() + &clean_vec.len().to_string() + " primes\n").as_bytes()).expect("Could not output count of primes");
    std::io::Write::write_all(&mut std::io::stdout(), ("It took ".to_owned() + &time.to_string() + "ms\n").as_bytes()).expect("Could not output duration");
}